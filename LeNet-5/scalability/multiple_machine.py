...
def worker_random_minibatches(minibatch, worker_minibatch_size, seed=1):
    worker_batches = []
...
def main():
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(nums_epoch):
            ...
            # move_acc = 0.8 * move_acc + 0.2 * delta_acc
            # biased_acc = move_acc / (1 - 0.8 ** epoch)
            ...
            if epoch % 1 == 0:
                print("After {} training epochs, training cost is {}, training accuracy is {:.4f}".format(epoch, epoch_cost, train_acc))

            # 每 5 轮输出一次在验证数据集上的测试结果
            # if epoch % 5 == 0:
            #     print("After {} training epochs, max test accuracy is {:.5f}".format(epoch, max_acc))

        # model_file = tf.train.latest_checkpoint("./save_model/")
        # saver.restore(sess, model_file)
        # test_acc = sess.run(accuracy, feed_dict=test_feed)
        # print("Max test accuracy is {}".format(test_acc))
