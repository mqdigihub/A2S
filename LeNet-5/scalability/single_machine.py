...
tf.app.flags.DEFINE_integer('mini_batch_size', 125, """Number of mini batch.""")
tf.app.flags.DEFINE_float('delta_acc', 0.0005, """The convergence condition.""")
    ...
    with tf.Session() as sess:
        ...
        while biased_acc > FLAGS.delta_acc:
            ...
            for mini_batch in mini_batches:
                (mini_batch_X, mini_batch_Y) = mini_batch                       # 分配worker训练的数据集分区
                ...
                paras = SGD_Optimizer(decay_lr, paras, grads)
                epoch_cost += cost / (num_mini_batch)

            decay_lr = FLAGS.lr * FLAGS.decay_rate**epoch
            ...
            if epoch % 1 == 0:
                print(
                    "After {} training epochs, training cost is {}, training accuracy is {:.5f} (sec), Biased acc is {}"
                      .format(epoch, epoch_cost, train_acc, biased_acc))

            # 每 5 轮输出一次在验证数据集上的测试结果
            if epoch % 5 == 0:
                print(
                    "After {} training epochs, validation accuracy is {:.5f}, test accuracy is {:.5f}"
                      .format(epoch, validate_acc, test_acc))

        test_acc = sess.run(accuracy, feed_dict=test_feed)
        print("The converged test accuracy is {}".format(test_acc))
