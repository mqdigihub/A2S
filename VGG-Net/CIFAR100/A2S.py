...
tf.app.flags.DEFINE_integer('grads_length', 9881430, """Number of gradients byte.""")
        ...
        while True:
            ...
            if grads != b'0x03':                    # 结束标志
                ...
                if str(socket) in self.sync_group:   # 同步组
                    ...
                    if len(socket_and_grads) == len(self.sync_group) or clock > 5:
                        clock = 0
        ...
        for i in range(len(socket_and_grads)):
            ...
            for j in range(self.l + 1):
                ...
                if j != self.l:
                    ...
                    self.grads["se" + str(j + 1) + "_dw1"] += g["se" + str(j + 1) + "_dw1"] * weight
                    self.grads["se" + str(j + 1) + "_dw2"] += g["se" + str(j + 1) + "_dw2"] * weight
                    self.grads["se" + str(j + 1) + "_db1"] += g["se" + str(j + 1) + "_db1"] * weight
                    self.grads["se" + str(j + 1) + "_db2"] += g["se" + str(j + 1) + "_db2"] * weight
...
def main():
    ...
    grads = utils.create_grads_dict(FLAGS.nums_worker)
