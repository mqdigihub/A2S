...
class A2S():
            ...
            if grads != b'0x03':                    # 结束标志
                ...
                if str(socket) in self.sync_group:     # 同步组
                    ...
                    if len(socket_and_grads) == self.stale_v or clock > 10:
                        clock = 0
                        self.sync_update(socket_and_grads, queue1)      # 同步更新
                ...
                if len(socket_and_grads) != 0:
                    self.sync_update(socket_and_grads, queue1)
                    socket_and_grads.clear()
    ...
    def sync_update(self, socket_and_grads, queue1):
        ...
        for i in range(len(socket_and_grads)):
            ...
            for j in range(self.l):
                self.grads['dw' + str(j + 1)] += g['dw' + str(j + 1)]
                self.grads['db' + str(j + 1)] += g['db' + str(j + 1)]
